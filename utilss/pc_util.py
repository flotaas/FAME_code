...
def random_sampling(pc, num_sample, replace=None, return_choices=False):
    ...
    '''
    pc: The input point cloud, represented as a 2D NumPy array where the first dimension (N) 
    corresponds to the number of points in the point cloud, and the second dimension (C) 
    represents the feature channels of each point (e.g., x, y, z coordinates, color, etc.).
    num_sample：从输入点云中采样的点数。
    replace：一个布尔值，表示是否应该替换取样。如果为 True，同一个点可以被多次选中；否则，所有采样点都是唯一的。
    如果没有指定替换，则会根据输入的大小和请求的样本数量自动确定。
    return_cchoices：布尔值，表示是否要将采样点的索引与采样点本身一起返回。
    '''
    if replace is None:
